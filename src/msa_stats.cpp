...
MSAStats::MSAStats(MSA msa)
{
  ...
  // correlation_2p = arma::field<arma::Mat<double>>(L, L);
  // correlation_3p = arma::field<arma::Cube<double>>(L, L, L);
  rel_entropy_grad_1p =
  ...
  for (int i = 0; i < L; i++) {
    ...
    // for (int j = 0; j < L; j++) {
      ...
      for (int m = 0; m < M; m++) {
        frequency_2p.at(i, j).at(*(align_ptr1 + m), *(align_ptr2 + m)) +=
          *(weight_ptr + m);
      }
    ...
    for (int j = i + 1; j < L; j++) {
      for (int k = j + 1; k < L; k++) {
        frequency_3p.at(i, j, k) = arma::Cube<double>(
          AA_ALPHABET_SIZE, AA_ALPHABET_SIZE, AA_ALPHABET_SIZE, arma::fill::zeros);

        int* align_ptr1 = msa.Alignment.colptr(i);
        int* align_ptr2 = msa.Alignment.colptr(j);
        int* align_ptr3 = msa.Alignment.colptr(k);
        for (int m = 0; m < M; m++) {
          frequency_3p.at(i, j, k).at(*(align_ptr1 + m), *(align_ptr2 + m), *(align_ptr3 + m)) +=
            *(weight_ptr + m);
        }
  ...
  //   // for (int j = 0; j < L; j++) {
  //   for (int j = i + 1; j < L; j++) {
  //     correlation_2p(i, j) = arma::Mat<double>(
  //       AA_ALPHABET_SIZE, AA_ALPHABET_SIZE);
  //     for (int aa1 = 0; aa1 < AA_ALPHABET_SIZE; aa1++) {
  //       for (int aa2 = 0; aa2 < AA_ALPHABET_SIZE; aa2++) {
  //         correlation_2p.at(i, j).at(aa1, aa2) = frequency_2p.at(i, j).at(aa1, aa2) - frequency_1p.at(aa1, i)*frequency_1p.at(aa2, j);
  //       }
  //     }
  //   }
  // }
  ...
  //   // for (int j = 0; j < L; j++) {
  //   //   for (int k = 0; k < L; k++) {
  //       correlation_3p(i, j, k) = arma::Cube<double>(
  ...
  //               frequency_1p.at(aa1, i)*frequency_1p.at(aa2, j)*frequency_1p.at(aa3, k);
  //           }
  //         }
  //       }
  //     }
        ...
        for (int aa1 = 0; aa1 < AA_ALPHABET_SIZE; aa1++) {
          for (int aa2 = 0; aa2 < AA_ALPHABET_SIZE; aa2++) {
            for (int aa3 = 0; aa3 < AA_ALPHABET_SIZE; aa3++) {
              ...
              output_stream << frequency_3p.at(i, j, k).at(aa1, aa2, aa3) << std::endl;
            }
          }
        }
        // output_stream << std::endl;
...
void
MSAStats::WriteCorrelation2pCompat(std::string output_file)
{
  std::ofstream output_stream(output_file);
  // output_stream << std::fixed << std::setprecision(6);
  ...
  for (int i = 0; i < L; i++) {
    ...
    // for (int j = 0; j < L; j++) {
      ...
      for (int aa1 = 0; aa1 < AA_ALPHABET_SIZE; aa1++) {
        for (int aa2 = 0; aa2 < AA_ALPHABET_SIZE; aa2++) {
          // output_stream << " " << frequency_2p.at(i, j).at(aa1, aa2) - frequency_1p.at(aa1, i)*frequency_1p.at(aa2, j);
          output_stream << frequency_2p.at(i, j).at(aa1, aa2) - frequency_1p.at(aa1, i)*frequency_1p.at(aa2, j) << std::endl;
          // output_stream << correlation_2p.at(i, j).at(aa1, aa2) << std::endl;
        }
      }
      // output_stream << std::endl;
    }
...
void
MSAStats::WriteCorrelation3pCompat(std::string output_file)
{
  std::ofstream output_stream(output_file);
  // output_stream << std::fixed << std::setprecision(6);
          ...
          for (int aa2 = 0; aa2 < AA_ALPHABET_SIZE; aa2++) {
            for (int aa3 = 0; aa3 < AA_ALPHABET_SIZE; aa3++) {
              output_stream <<
                ...
                frequency_2p.at(i, k).at(aa1, aa3)*frequency_1p.at(aa2, j) -
                frequency_2p.at(j, k).at(aa2, aa3)*frequency_1p.at(aa1, i) +
                2.0*frequency_1p.at(aa1, i)*frequency_1p.at(aa2, j)*frequency_1p.at(aa3, k) << std::endl;
            }
          }
        }
      }
