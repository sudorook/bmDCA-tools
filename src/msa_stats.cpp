...
MSAStats::MSAStats(MSA *msa)
{
  ...
  correlation_2p = arma::field<arma::Mat<double>>(N, N);

  rel_entropy_1p = arma::Mat<double>(Q, N, arma::fill::zeros);
  rel_entropy_1p_pos = arma::Col<double>(N, arma::fill::zeros);
...
#pragma omp for schedule(dynamic,1)
    for (int i = 0; i < N; i++) {
      for (int j = i + 1; j < N; j++) {
        correlation_2p(i, j) = arma::Mat<double>(Q, Q, arma::fill::zeros);
        for (int aa1 = 0; aa1 < Q; aa1++) {
          for (int aa2 = 0; aa2 < Q; aa2++) {
            correlation_2p(i, j)(aa1, aa2) =
              frequency_2p(i, j)(aa1, aa2) -
              frequency_1p(aa1, i) * frequency_1p(aa2, j);
          }
        }
      }
    }
  ...
  for (int i = 0; i < N; i++) {
    for (int aa = 0; aa < Q; aa++) {
      ...
      if (pos_freq < 1. && pos_freq > 0.) {
        rel_entropy_1p_pos(i) += pos_freq * log(pos_freq / background_freq);
        rel_entropy_1p(aa, i) =
...
// {
//   rel_entropy_1p_pos.save(output_file, arma::arma_binary);
// };
...
MSAStats::writeRelEntropyPosAscii(std::string output_file)
{
  std::ofstream output_stream(output_file);
  for (int i = 0; i < N; i++) {
    output_stream << " " << rel_entropy_1p_pos(i) << std::endl;
  }
};
...
{
  correlation_2p.save(output_file, arma::arma_binary);
};
...
MSAStats::writeCorrelation2pAscii(std::string output_file)
{
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      ...
      for (int aa1 = 0; aa1 < Q; aa1++) {
        for (int aa2 = 0; aa2 < Q; aa2++) {
          output_stream << " " << correlation_2p(i, j)(aa1, aa2);
        }
      }
      output_stream << std::endl;
    }
