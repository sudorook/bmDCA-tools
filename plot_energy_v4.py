...
def parse_options():
    ...
    parser.add_argument(
        "-c", "--mcmc1", dest="mcmc1", required=True, help="mcmc file 1"
    )
    parser.add_argument(
        "-z", "--mcmc2", dest="mcmc2", required=True, help="mcmc file 2"
    )
    parser.add_argument(
...
def main():
    ...
    mcmc1_seqs = load_sequences(options.mcmc1)
    mcmc2_seqs = load_sequences(options.mcmc2)

    msa_energies = compute_energies(msa_seqs, h, J)
    mcmc1_energies = compute_energies(mcmc1_seqs, h, J)
    mcmc2_energies = compute_energies(mcmc2_seqs, h, J)
    ...
    with plt.style.context("fivethirtyeight"):
        ...
        plt.hist(
            mcmc1_energies - standard_energy,
            alpha=0.5,
            label="MCMC (MH)",
            density=True,
        )
        plt.hist(
            mcmc2_energies - standard_energy,
            alpha=0.5,
            label="MCMC (Z)",
            density=True,
        )
